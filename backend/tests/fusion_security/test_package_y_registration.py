"""Fictional Y registrations retain old objects and reject ambiguous bindings."""
import asyncio
...
from src.fusion.package_y_registration import register_y_content
...
def test_y_registration_preserves_input_packages_and_prior_entry_identity():
    ...
    result = register_y_content(package, doc, previous_package=previous,
    ...
    assert result['single_player_content'][key].keys() == {'Y'}
    assert result['single_player_required'][key] == {'Y'}
    assert (key, 'Y') not in result['single_player_content']
    catalogue = resolve_role_content(result['single_player_content'], key, 'Y')
    assert catalogue.character_id == 'Y' and catalogue.package_hash == key
...
def test_y_registration_opens_private_book_and_exposes_only_current_other_role_options(play):
    previous, package, doc = fixture()
    kwargs = register_y_content(package, doc, previous_package=previous)
    ...
    opening = runtime_service.create(request(release['id'], 'Y'), 1)
    assert opening['selected_character_id'] == 'Y'
    ...
    view = play.play.create({'opening_session_id': opening['session_id'], 'idempotency_key': 'start-y'}, 1)
    assert view['single_player']['topics'] == []
    assert set(kwargs['single_player_content'][content_hash(package)]['Y'].stages) == {p['id'] for p in package['phases']}
...
def test_registered_y_without_its_catalogue_fails_closed_in_opening_and_play(play):
    previous, package, doc = fixture()
    kwargs = register_y_content(package, doc, previous_package=previous)
    release = publish(play, package)
    opening_service = PackageRuntimeService(play.db, play.publisher, **kwargs)
    opening = opening_service.create(request(release['id'], 'Y'), 1)
    service = PackagePlayService(play.db, play.publisher, play.model, play.policy, **kwargs)
    view = service.create({'opening_session_id': opening['session_id'], 'idempotency_key': 'start-y'}, 1)
    ...
    with pytest.raises(PackageRuntimeError, match='PACKAGE_OPENING_SINGLE_PLAYER_UNAVAILABLE'):
        missing.create(request(release['id'], 'Y', 'second-y'), 1)
...
def test_y_registration_refuses_any_existing_target_key_without_overwriting(target):
    previous, package, doc = fixture()
    key = content_hash(package)
    ...
    with pytest.raises(ValueError, match='Y_REGISTRATION_ALREADY_EXISTS'):
        register_y_content(package, doc, previous_package=previous,
                           single_player_content=contents, single_player_required=required)
...
def test_y_registration_rejects_invalid_directory_binding_or_content(mutation):
    previous, package, doc = fixture()
    ...
    elif mutation == 'human-responder': doc['topics'][0]['responders'][0]['character_id'] = 'Y'
...
def test_y_registration_requires_valid_packages_with_only_new_content_version(mutation):
    previous, package, doc = fixture()
    ...
    with pytest.raises(ValueError, match='Y_REGISTRATION_'):
        register_y_content(package, doc, previous_package=previous)


@pytest.mark.parametrize('registry', [[], 'not-a-registry', 1])
def test_y_registration_rejects_non_mapping_registries(registry):
    previous, package, doc = fixture()
    ...
    for keyword in ('single_player_content', 'single_player_required'):
        with pytest.raises(ValueError, match='Y_REGISTRATION_REGISTRY_INVALID'):
            register_y_content(package, doc, previous_package=previous, **{keyword: registry})
