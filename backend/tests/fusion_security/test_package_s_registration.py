"""Fictional S registrations retain old objects and reject ambiguous bindings."""
import asyncio
...
from src.fusion.package_s_registration import register_s_content
...
def fixture():
    def rename(value):
        ...
        return 'S' if value == 'a' else value
    previous = rename(full_package())
    previous['knowledge'][1]['retelling'] = 'MAY_RETELL'
    package = deepcopy(previous)
    package['content_version'] = 's-offline-v2'
    doc = document(package)
    doc['selected_character_id'] = 'S'
    doc['source_package_hash'] = content_hash(package)
    return previous, package, doc


def test_s_registration_preserves_input_packages_and_prior_entry_identity():
    previous, package, doc = fixture()
    old_legacy = object()
    old_nested = {'T': object(), 'J': object(), 'H': object()}
    old_required = {'T', 'J', 'H'}
    contents = {'legacy-package': old_legacy, 'tjh-package': old_nested}
    required = {'legacy-package': 'T', 'tjh-package': old_required}
    inputs = canonical_json([previous, package, doc])
    result = register_s_content(package, doc, previous_package=previous,
    ...
    assert contents == {'legacy-package': old_legacy, 'tjh-package': old_nested}
    assert required == {'legacy-package': 'T', 'tjh-package': old_required}
    ...
    assert result['single_player_content'][key].keys() == {'S'}
    assert result['single_player_required'][key] == {'S'}
    assert (key, 'S') not in result['single_player_content']
    catalogue = resolve_role_content(result['single_player_content'], key, 'S')
    assert catalogue.character_id == 'S' and catalogue.package_hash == key
...
def test_s_registration_opens_private_book_and_exposes_only_current_other_role_options(play):
    previous, package, doc = fixture()
    kwargs = register_s_content(package, doc, previous_package=previous)
    ...
    opening = runtime_service.create(request(release['id'], 'S'), 1)
    assert opening['selected_character_id'] == 'S'
    ...
    view = play.play.create({'opening_session_id': opening['session_id'], 'idempotency_key': 'start-s'}, 1)
    assert view['single_player']['topics'] == []
    assert set(kwargs['single_player_content'][content_hash(package)]['S'].stages) == {p['id'] for p in package['phases']}
...
def test_registered_s_without_its_catalogue_fails_closed_in_opening_and_play(play):
    previous, package, doc = fixture()
    kwargs = register_s_content(package, doc, previous_package=previous)
    release = publish(play, package)
    opening_service = PackageRuntimeService(play.db, play.publisher, **kwargs)
    opening = opening_service.create(request(release['id'], 'S'), 1)
    service = PackagePlayService(play.db, play.publisher, play.model, play.policy, **kwargs)
    view = service.create({'opening_session_id': opening['session_id'], 'idempotency_key': 'start-s'}, 1)
    ...
    with pytest.raises(PackageRuntimeError, match='PACKAGE_OPENING_SINGLE_PLAYER_UNAVAILABLE'):
        missing.create(request(release['id'], 'S', 'second-s'), 1)
...
def test_s_registration_refuses_any_existing_target_key_without_overwriting(target):
    previous, package, doc = fixture()
    key = content_hash(package)
    ...
    with pytest.raises(ValueError, match='S_REGISTRATION_ALREADY_EXISTS'):
        register_s_content(package, doc, previous_package=previous,
                           single_player_content=contents, single_player_required=required)
...
def test_s_registration_rejects_invalid_directory_binding_or_content(mutation):
    previous, package, doc = fixture()
    ...
    elif mutation == 'human-responder': doc['topics'][0]['responders'][0]['character_id'] = 'S'
...
def test_s_registration_requires_valid_packages_with_only_new_content_version(mutation):
    previous, package, doc = fixture()
    ...
    with pytest.raises(ValueError, match='S_REGISTRATION_'):
        register_s_content(package, doc, previous_package=previous)


@pytest.mark.parametrize('registry', [[], 'not-a-registry', 1])
def test_s_registration_rejects_non_mapping_registries(registry):
    previous, package, doc = fixture()
    ...
    for keyword in ('single_player_content', 'single_player_required'):
        with pytest.raises(ValueError, match='S_REGISTRATION_REGISTRY_INVALID'):
            register_s_content(package, doc, previous_package=previous, **{keyword: registry})
