"""Build an isolated S catalogue registration without touching prior entries.
...
def register_s_content(package, document, *, previous_package,
                       single_player_content=None, single_player_required=None):
    """Return independent outer registries for a new, version-only S package."""
    if (not validate_package(package)['valid']
            or not validate_package(previous_package)['valid']):
        raise ValueError('S_REGISTRATION_PACKAGE_INVALID')
    ...
    if (package['content_version'] == previous_package['content_version']
        ...
        raise ValueError('S_REGISTRATION_NEW_VERSION_REQUIRED')
    if ({key: value for key, value in package.items() if key != 'content_version'}
        ...
        raise ValueError('S_REGISTRATION_PACKAGE_CONTENT_CHANGED')
    if (not isinstance(document, dict) or document.get('selected_character_id') != 'S'
            or document.get('package_hash') != package_hash
            or document.get('source_package_hash') != package_hash):
        raise ValueError('S_REGISTRATION_DOCUMENT_BINDING_INVALID')
    for registry in (single_player_content, single_player_required):
        if registry is not None and not isinstance(registry, Mapping):
            raise ValueError('S_REGISTRATION_REGISTRY_INVALID')
        if registry is not None and package_hash in registry:
            raise ValueError('S_REGISTRATION_ALREADY_EXISTS')
    ...
    contents[package_hash] = {'S': catalogue}
    required[package_hash] = {'S'}
